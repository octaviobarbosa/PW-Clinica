...
namespace PW_Clinica.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, [Bind(Include = "IdCidade,Nome,CPF,CRM_CRN,Especialidade,Logradouro,Numero,Bairro,CEP,Estado,DDD1,DDD2,Telefone1,Telefone2")] tbProfissional tbProfissional, [Bind(Include = "IdPlano")] tbContrato tbContrato, [Bind(Include = "IdCidade")] tbCidade tbCidade)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    iur.RoleId = model.TipoProfissional;
                    iur.UserId = user.Id;
                    user.Roles.Add(iur);
                    var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
